...
| [`test_walk_plan_agent_schema.py`](./test_walk_plan_agent_schema.py) | **US-01, US-06, US-16** | • Pydantic V2 산책 생성 요청 엄격 검증(시간 5~120분, 좌표 유효범위)<br>• 필수 파라미터 누락 시 Clarification(되물음) 트리거<br>• 반려견 건강 프로필(슬개골 탈구 단계) 및 롱텀 메모리 주입<br>• 보행 속도 매핑 기반 목표 거리 자동 환산 검증 |
