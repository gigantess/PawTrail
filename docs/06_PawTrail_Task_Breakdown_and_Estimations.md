...
|:---:|---|---|:---:|
| **Member A** | **AI Agent / Product Logic Lead** | LangGraph Agent 상태 머신, Pydantic V2 Strict Schema, 무상태 프롬프트 오케스트레이션, 경로 평가 브리핑 | **52 h** |
| **Member B** | **Vision / Multimodal AI Lead** | Gemini 1.5 Flash Vision 현장 위험물(턱/계단/장애물) 분석, Structured JSON 규격화, 저조도/블러 예외 처리 | **28 h** |
| **Member C** | **Backend / Routing / GIS Lead** | FastAPI REST API, Routing API Adapter (ORS/OSRM), OSM Steps 배제, DEM 경사도, SunCalc 그늘 분석, Candidate Scorer | **126 h** |
| **Member D** | **Frontend / Mobile App Lead** | React Native (Expo SDK 51+), Maps Polyline 색상 분기, Android Foreground Service GPS, expo-speech TTS 음성 안내, AsyncStorage | **127 h** |
| **Member E** | **Infra / DevOps & QA Lead** | EAS Build APK 패키징, EAS Update 무선 OTA 파이프라인, Supabase Auth/DB, 200m 마스킹, 기상청 단기예보 n8n 연동, 5인 CBT 총괄 | **39 h** |
...
| **TASK-A2-2** | 반려견 프로필 온보딩 및 수정 폼 UI 컴포넌트 개발 | Member D | 4 h | Week 1 | 견종, 체중, 연령, 관절 안심 수준(0~2단계) 입력 인터페이스 구현 |
| **TASK-A2-3** | 기기 변경 및 앱 재설치 대비 프로필 JSON 파일 내보내기/가져오기 모듈 구현 | Member D | 3 h | Week 2 | `expo-file-system` / `expo-sharing` 연동 로컬 백업/복원 검증 |
| **TASK-A2-4** | 앱 온보딩 및 프로필 UI 전역 웰니스 카피라이팅 적용 및 정적 검증 | Member D | 3 h | Week 2 | 질병 용어(슬개골 탈구 등) 노출 0건 검출 및 UI 텍스트 순화 검증 |
...
* **담당자**: **Member C (Lead / GIS), Member A (Agent), Member D (UI)**

| Task ID | 세부 작업 내용 (Task Specification) | 담당자 | 공수 | 주차 | 완료 정의 및 산출물 (DoD / Deliverables) |
|:---:|---|:---:|:---:|:---:|---|
| **TASK-A3-1** | 플래너 UI 산책 시간 슬라이더(10~90분) 및 원터치 칩(15/30/45/60분) 개발 | Member D | 4 h | Week 1 | 직관적인 시간 선택 슬라이더 및 프리셋 칩 UI 컴포넌트 구현 |
| **TASK-A3-2** | 체급(소/중/대/노령견)별 표준 보행 속도 상수 모델 및 목표 거리 자동 환산 엔진 구현 | Member C | 5 h | Week 2 | 2.8/3.6/4.2/2.2 km/h 속도 상수 기반 목표 거리(km) 환산 함수 구현 |
...
| **TASK-B1-4** | 계단 밀집 구역 테스트베드(5개 권역) 대상 무계단 회피 라우팅 정량 검증 | Member C | 8 h | Week 2 | 테스트 케이스 계단 배제 적중률 100% 달성 단위/통합 테스트 |
...
| Task ID | 세부 작업 내용 (Task Specification) | 담당자 | 공수 | 주차 | 완료 정의 및 산출물 (DoD / Deliverables) |
|:---:|---|:---:|:---:|:---:|---|
| **TASK-B2-1** | 수치표고모델(DEM) 래스터 데이터 적재 및 경로 세그먼트 고도 샘플링 파이프라인 | Member C | 7 h | Week 2 | GeoTIFF DEM 래스터 좌표계 매핑 및 고도 보간 파이프라인 구축 |
...
| **TASK-B2-4** | 구릉지 지형 테스트베드 대상 완만 경로 선별 알고리즘 정확도 검증 | Member C | 4 h | Week 2 | 가파른 지형 샘플 10개 대상 완만 경로 선별 유효성 검증 |
...
| Task ID | 세부 작업 내용 (Task Specification) | 담당자 | 공수 | 주차 | 완료 정의 및 산출물 (DoD / Deliverables) |
|:---:|---|:---:|:---:|:---:|---|
| **TASK-B3-1** | `SunCalc` 연동 산책 일시/위치 기준 실시간 태양 고도각 및 방위각 연산 모듈 개발 | Member C | 5 h | Week 2 | 시간대별 태양 위치 좌표 수리 모델 구현 및 정확도 검증 |
...
| **TASK-B3-4** | 정오 vs 늦은 오후 시간대별 그늘 평가 대조 테스트베드 검증 및 신뢰도 태깅 | Member C | 4 h | Week 3 | 시간대별 그늘 비율 변동성 대조 테스트 및 신뢰도 메타데이터 부여 |
...
* **담당자**: **Member C (Lead / GIS), Member A (Agent)**
...
| **TASK-B4-3** | 계단·경사·그늘·거리 다요소 Candidate Route Scorer 수치 랭킹 알고리즘 구현 | Member C | 8 h | Week 2 | 다요소 정규화 가중치 합산 기반 코스 순위화 스코어러 완성 |
...
| **TASK-C1-3** | 거리, 시간, 최대 경사, 그늘 비율 요약 카드 및 OSRM 스텝 리스트 바텀시트 UI 개발 | Member D | 6 h | Week 3 | 코스 프리뷰 카드 및 스와이프 가능한 회전 안내 리스트 뷰 제작 |
| **TASK-C1-4** | 모바일 실기기 뷰포트 반응형 레이아웃 구성 및 지도 터치 제스처 최적화 | Member D | 4 h | Week 3 | 다양한 해상도(Android/iOS) 실기기 레이아웃 깨짐 0건 검증 |
...
| Task ID | 세부 작업 내용 (Task Specification) | 담당자 | 공수 | 주차 | 완료 정의 및 산출물 (DoD / Deliverables) |
|:---:|---|:---:|:---:|:---:|---|
| **TASK-E2-1** | 산책 완주 결과 요약 인포그래픽 모달 컴포넌트 개발 (시간/거리/소모칼로리) | Member D | 4 h | Week 4 | 산책 완주 요약 통계 카드 및 축하 인포그래픽 애니메이션 뷰 제작 |
| **TASK-E2-2** | 보행 만족도 점수(별점) 및 체감 태그(계단 없음, 경사 완만, 그늘 충분) 입력 UI | Member D | 4 h | Week 4 | 원터치 피드백 태그 선택 컴포넌트 및 만족도 수집 폼 구현 |
...
* **담당자**: **Member E (Lead / Infra), Member C (Backend), Member D (UI)**

| Task ID | 세부 작업 내용 (Task Specification) | 담당자 | 공수 | 주차 | 완료 정의 및 산출물 (DoD / Deliverables) |
|:---:|---|:---:|:---:|:---:|---|
| **TASK-F1-1** | 기상청 단기예보 API 클라이언트 및 n8n/스케줄러 기반 시간대별 기온/운량 파싱 | Member E | 4 h | Week 4 | 기상청 격자 좌표 변환 및 매시간 기온, 풍속, 하늘상태 파싱 파이프라인 |
| **TASK-F1-2** | 시간대별 일사량 및 지면열 위험 지수(안전/주의/위험) 산출 수지식 모듈 구현 | Member E | 3 h | Week 4 | 일사량 회귀식을 통한 지면온도 추정 및 3단계 열 위험도 산출 모듈 |
...
| **TASK-F1-4** | 플래너 상단 산책 골든타임 알림 카드 UI 렌더링 및 위험 시간대 경고 배너 개발 | Member D | 3 h | Week 4 | 35℃ 이하 안심 시간대 골든타임 표출 카드 및 위험 경고 배너 구현 |
...
* **담당자**: **Member C (Lead / GIS), Member D (UI)**

| Task ID | 세부 작업 내용 (Task Specification) | 담당자 | 공수 | 주차 | 완료 정의 및 산출물 (DoD / Deliverables) |
|:---:|---|:---:|:---:|:---:|---|
| **TASK-G1-1** | 전국 공영주차장 공공데이터 API 클라이언트 및 좌표 반경 검색 모듈 구현 | Member C | 4 h | Week 4 | 출발지 1km 반경 내 무료/유료 공영주차장 위치 및 요금 정보 조회 |
...
| **TASK-G1-3** | 플래너 내 '주차 거점 산책' 토글 및 인근 주차장 선택 바텀시트 UI 개발 | Member D | 3 h | Week 4 | 주차 거점 모드 전환 스위치 및 주차장 선택 인터랙션 컴포넌트 구현 |
...
* **담당자**: **Member E (Lead / Infra), Member D (UI)**

| Task ID | 세부 작업 내용 (Task Specification) | 담당자 | 공수 | 주차 | 완료 정의 및 산출물 (DoD / Deliverables) |
|:---:|---|:---:|:---:|:---:|---|
| **TASK-G2-1** | Supabase Auth 기반 간편 이메일 가입/로그인 모듈 구축 (Auto-confirm 활성화) | Member E | 4 h | Week 4 | 이메일/비밀번호 간편 가입, JWT 세션 발급 및 소셜 OAuth 의존 배제 |
| **TASK-G2-2** | 커뮤니티 코스 공유 시 출발지/도착지 200m 좌표 공간 블러링(Spatial Jittering) 백엔드 파이프라인 | Member E | 4 h | Week 4 | 출발/도착 반경 200m 좌표 절단 및 가우스 노이즈 지터링으로 자택 은닉 |
| **TASK-G2-3** | Supabase `community_courses` 테이블 적재 및 공개 피드 조회 REST API 구현 | Member E | 4 h | Week 4 | 200m 마스킹된 공개 안심 코스 피드 페이징 조회 API 개발 |
| **TASK-G2-4** | 커뮤니티 피드 뷰, 200m 마스킹 사전확인 모달 및 코스 북마크/좋아요 UI 개발 | Member D | 4 h | Week 4 | 마스킹된 코스 프리뷰 모달, 안심 공유 동의 다이얼로그 및 북마크 뷰 제작 |
...
* **담당자**: **Member E (Lead / Infra), Member D (Mobile Hotfix)**

| Task ID | 세부 작업 내용 (Task Specification) | 담당자 | 공수 | 주차 | 완료 정의 및 산출물 (DoD / Deliverables) |
|:---:|---|:---:|:---:|:---:|---|
| **TASK-H1-1** | EAS Build 기반 테스터용 Android APK 1회 패키징 및 5인 테스터 배포 환경 구축 | Member E | 6 h | Week 4 | EAS Build 클라우드 프로파일 설정, 릴리즈 APK 빌드 및 테스터 5인 설치 |
| **TASK-H1-2** | EAS Update (`expo-updates`) GitHub Actions CI/CD 연동 무선 무점검 OTA 파이프라인 수립 | Member E | 6 h | Week 4 | `main` 브랜치 푸시 시 재설치 없는 무선 무점검 OTA 자동 배포 검증 |
| **TASK-H1-3** | 실사용자 5인 대상 필드 CBT 테스트 총괄 운영 및 도메인별 피드백 수집·종합 평가 보고서 작성 | Member E | 6 h | Week 5 | 5인 견주 산책 시나리오 완주 설문 수집, 만족도 및 결함 분석 보고서 정리 |
| **TASK-H1-4** | 5인 CBT 피드백 기반 핵심 모바일 UX/음성 안내 타이밍 튜닝 및 실시간 OTA 핫픽스 배포 | Member D | 6 h | Week 5 | 음성 안내 타이밍(30m 전) 캘리브레이션 및 EAS Update 무선 핫픽스 배포 |
...
|:---:|---|:---:|:---:|:---:|:---:|
| **Member A** | **AI Agent / Product Logic Lead** | 9개 Task | **52 h** | 약 10.4 h / 주 | 14.0% |
| **Member B** | **Vision / Multimodal AI Lead** | 5개 Task | **28 h** | 약 5.6 h / 주 | 7.5% |
| **Member C** | **Backend / Routing / GIS Data Lead** | 19개 Task | **126 h** | 약 25.2 h / 주 | 33.9% |
| **Member D** | **Frontend / Mobile App Lead** | 26개 Task | **127 h** | 약 25.4 h / 주 | 34.1% |
| **Member E** | **Infra / DevOps & QA Lead** | 9개 Task | **39 h** | 약 7.8 h / 주 | 10.5% |
| **합계** | **5인 전원 (Full Agile Team)** | **68개 Task** | **372 h** | **약 74.4 h / 주 (팀 전체)** | **100.0%** |
...
```text
[Sprint 1: Week 1~2] 코어 AI 파이프라인, 로컬 스토리지 & 도메인 GIS 엔진 (166 h)
       │
[Sprint 2: Week 3~4] 모바일 연동, 핸즈프리 음성 안내, 비전 판독, 커뮤니티 & EAS 배포 (182 h)
       │
[Hardening: Week 5] 5인 실사용자 CBT 완료 보고, 무선 OTA 실시간 핫픽스 & 최종 안정화 (24 h)
```

| 스프린트 단계 | 주차 (Timeline) | 주간 핵심 마일스톤 | 투입 공수 (Hours) | 주요 산출물 |
|:---:|:---:|---|:---:|---|
| **Sprint 1** | **Week 1** | Expo 프로젝트 초기화, Supabase Auth 연동, LangGraph 스키마 수립, Routing 어댑터 | **70 h** | `AgentState`, ORS/OSRM Adapter, 슬라이더 UI |
| **Sprint 1** | **Week 2** | `AsyncStorage` 프로필 완성, OSM Steps 배제, DEM 경사도 연산, Candidate Scorer | **96 h** | 무계단 라우팅, 경사도 비용 함수, 다요소 채점기 |
| **Sprint 2** | **Week 3** | SunCalc 그늘 분석, Gemini 1.5 Flash 비전 판독, react-native-maps & expo-speech 음성 길 안내 | **104 h** | 그늘 다각형 교차, 비전 검사기, 백그라운드 TTS |
| **Sprint 2** | **Week 4** | Android Foreground Service GPS, 주차장 연동, 200m 마스킹 커뮤니티, EAS Build APK 패키징 | **78 h** | Foreground 추적기, 공간 지터링, 테스터 APK |
| **Hardening** | **Week 5** | 5인 CBT 필드 테스트 설문 분석, EAS Update 무선 OTA 핫픽스 배포, 최종 안정화 | **24 h** | CBT 종합 평가서, 무선 OTA 실시간 반영 |
...
| **US-A2** | 반려견 프로필 로컬 등록 및 JSON 백업/복원 | Member D | — | 3 pt | 4개 | 14 h | Sprint 1 (Week 1~2) |
| **US-A3** | 목표 산책 시간/거리 기반 환산 (속도/슬라이더) | Member C | Member A, D | 3 pt | 3개 | 14 h | Sprint 1 (Week 1~2) |
| **US-B1** | 지도 데이터 기반 계단 구간 우선 회피 | Member C | — | 8 pt | 4개 | 34 h | Sprint 1 (Week 2) |
| **US-B2** | DEM 기반 완만 경사 우선 순환 경로 도출 | Member C | — | 5 pt | 4개 | 24 h | Sprint 1 (Week 2) |
| **US-B3** | 태양 위치 및 건물 형상 기반 시간대별 그늘 평가 | Member C | — | 5 pt | 4개 | 22 h | Sprint 1~2 (Week 2~3) |
| **US-B4** | Routing API Adapter 연동 및 다요소 스코어링 | Member C | Member A | 8 pt | 4개 | 34 h | Sprint 1 (Week 1~2) |
| **US-C1** | React Native Maps 기반 구간별 색상 분기 렌더링 | Member D | — | 5 pt | 4개 | 24 h | Sprint 2 (Week 3) |
...
| **US-E2** | 산책 종료 후 보행 체감 피드백 수집 및 로컬 통계 | Member D | — | 3 pt | 3개 | 12 h | Sprint 2 (Week 4) |
| **US-E3** | 로컬 누적 피드백 기반 무상태(Stateless) AI 추천 보정 | Member A | Member D | 3 pt | 3개 | 14 h | Sprint 2 (Week 4) |
| **US-F1** | 기상청 단기예보 연동 시간대별 열 위험 지수 안내 | Member E | Member C, D | 3 pt | 4개 | 12 h | Sprint 2 (Week 4) |
| **US-G1** | 출발 거점 연계 공영주차장(P&R) 코스 탐색 | Member C | Member D | 2 pt | 3개 | 10 h | Sprint 2 (Week 4) |
| **US-G2** | 간편 이메일 가입 및 코스 공유 (출발지 200m 마스킹) | Member E | Member D | 4 pt | 4개 | 16 h | Sprint 2 (Week 4) |
...
### 5.1 Member A (AI Agent & Product Logic Lead)
* **총 배정 공수**: **52 Hours** (전체 공수의 14.0% | 주당 평균 약 10.4h)
...
| **소계** | **5개 스토리** | **Member A 전담 Task 9개 합계** | **52 h** | — |

---

### 5.2 Member B (Vision / Multimodal AI Lead)
* **총 배정 공수**: **28 Hours** (전체 공수의 7.5% | 주당 평균 약 5.6h)
* **담당 스토리**: US-D1, US-D2
* **전담 Task 명세**:
...
| **소계** | **2개 스토리** | **Member B 전담 Task 5개 합계** | **28 h** | — |

---

### 5.3 Member C (Backend / Routing / GIS Data Lead)
* **총 배정 공수**: **126 Hours** (전체 공수의 33.9% | 주당 평균 약 25.2h)
* **담당 스토리**: US-A3, US-B1, US-B2, US-B3, US-B4, US-D2, US-G1
* **전담 Task 명세**:
...
| **소계** | **7개 스토리** | **Member C 전담 Task 19개 합계** | **126 h** | — |

---

### 5.4 Member D (Frontend / Mobile App Lead)
* **총 배정 공수**: **127 Hours** (전체 공수의 34.1% | 주당 평균 약 25.4h)
* **담당 스토리**: US-A2, US-A3, US-C1, US-C2, US-D2, US-E1, US-E2, US-E3, US-F1, US-G1, US-G2, US-H1
* **전담 Task 명세**:

| Task ID | 소속 스토리 | 세부 작업 내용 | 공수 | 담당 주차 |
|:---:|:---:|---|:---:|:---:|
| **TASK-A2-1** | US-A2 | `@react-native-async-storage/async-storage` 래퍼 및 프로필 CRUD 모듈 개발 | 4 h | Week 1 |
| **TASK-A2-2** | US-A2 | 반려견 프로필 온보딩 및 수정 폼 UI 컴포넌트 개발 | 4 h | Week 1 |
| **TASK-A2-3** | US-A2 | 기기 변경 및 앱 재설치 대비 프로필 JSON 파일 내보내기/가져오기 모듈 구현 | 3 h | Week 2 |
| **TASK-A2-4** | US-A2 | 앱 온보딩 및 프로필 UI 전역 웰니스 카피라이팅 적용 및 정적 검증 | 3 h | Week 2 |
| **TASK-A3-1** | US-A3 | 플래너 UI 산책 시간 슬라이더(10~90분) 및 원터치 칩(15/30/45/60분) 개발 | 4 h | Week 1 |
...
| **TASK-C1-3** | US-C1 | 거리, 시간, 최대 경사, 그늘 비율 요약 카드 및 OSRM 스텝 리스트 바텀시트 UI 개발 | 6 h | Week 3 |
| **TASK-C1-4** | US-C1 | 모바일 실기기 뷰포트 반응형 레이아웃 구성 및 지도 터치 제스처 최적화 | 4 h | Week 3 |
...
| **TASK-E2-1** | US-E2 | 산책 완주 결과 요약 인포그래픽 모달 컴포넌트 개발 (시간/거리/소모칼로리) | 4 h | Week 4 |
| **TASK-E2-2** | US-E2 | 보행 만족도 점수(별점) 및 체감 태그(계단 없음, 경사 완만, 그늘 충분) 입력 UI | 4 h | Week 4 |
...
| **TASK-F1-4** | US-F1 | 플래너 상단 산책 골든타임 알림 카드 UI 렌더링 및 위험 시간대 경고 배너 개발 | 3 h | Week 4 |
| **TASK-G1-3** | US-G1 | 플래너 내 '주차 거점 산책' 토글 및 인근 주차장 선택 바텀시트 UI 개발 | 3 h | Week 4 |
| **TASK-G2-4** | US-G2 | 커뮤니티 피드 뷰, 200m 마스킹 사전확인 모달 및 코스 북마크/좋아요 UI 개발 | 4 h | Week 4 |
| **TASK-H1-4** | US-H1 | 5인 CBT 피드백 기반 핵심 모바일 UX/음성 안내 타이밍 튜닝 및 실시간 OTA 핫픽스 배포 | 6 h | Week 5 |
| **소계** | **12개 스토리** | **Member D 전담 Task 26개 합계** | **127 h** | — |

---

### 5.5 Member E (Infra / DevOps & QA Lead)
* **총 배정 공수**: **39 Hours** (전체 공수의 10.5% | 주당 평균 약 7.8h)
* **담당 스토리**: US-F1, US-G2, US-H1
* **전담 Task 명세**:
...
| **TASK-H1-3** | US-H1 | 실사용자 5인 대상 필드 CBT 테스트 총괄 운영 및 도메인별 피드백 수집·종합 평가 보고서 작성 | 6 h | Week 5 |
| **소계** | **3개 스토리** | **Member E 전담 Task 9개 합계** | **39 h** | — |
...
* **총 개발 공수**: **372 Hours**  
  $$\text{Member A}(52\text{h}) + \text{Member B}(28\text{h}) + \text{Member C}(126\text{h}) + \text{Member D}(127\text{h}) + \text{Member E}(39\text{h}) = \mathbf{372\text{ Hours}}$$
* **생명주기 산출물 정합성**:
