...
* **프로젝트명**: PawTrail (AI Native 반려견 안심 산책 에이전트 및 프라이버시 우선 플랫폼)
* **목적**: 18개 사용자 스토리(US-A1 ~ US-H1)를 실제 개발 가능한 세부 Task로 분할하고, 애자일 추정치(Story Points, 공수 Hours) 및 담당자를 지정하여 5주간의 명확한 실행 계획을 제공.
* **아키텍처 정책 반영**:
  - **소셜 로그인 배제 & 간편 이메일 가입**: Supabase Auth 기본 이메일/비밀번호 가입(Auto-confirm)으로 외부 의존성 및 심사 병목 원천 제거.
  - **개인정보 로컬 보관 (Local-First)**: 반려견 프로필, 상세 GPS 궤적, 자택 출발지는 폰(IndexedDB)에만 저장하고 JSON 백업/복원 지원.
  - **커뮤니티 최소 공유**: 커뮤니티 공개 코스에만 출발지 200m 마스킹 블러링 적용 후 업로드.
* **팀 구성 (5인)**:
...
* **세부 Task**:
  * **TASK-A2-1**: IndexedDB 스키마 및 LocalForage 래퍼 라이브러리 연동 (4h / Member D)
  * **TASK-A2-2**: 반려견 프로필(견종, 체중, 연령, 관절 상태) 입력/수정 폼 컴포넌트 개발 (4h / Member D)
  * **TASK-A2-3**: 브라우저 캐시 삭제 대비 "프로필 JSON 파일 내보내기/가져오기" 백업 모듈 구현 (6h / Member D)
...
* **담당**: Member D | **스프린트**: Sprint 2 (Week 3)
* **세부 Task**:
  * **TASK-C2-1**: 추천 경로 주요 Waypoint 좌표 기반 네이버 지도 도보 길찾기 딥링크 생성 (4h / Member D)
  * **TASK-C2-2**: 카카오맵 도보 길찾기 URL Scheme 생성 및 원터치 버튼 연동 (4h / Member D)
...
#### US-E1: Foreground 위치 추적 및 실산책 경로 로컬 저장 (5 pt / 총 22h)
...
* **세부 Task**:
  ...
  * **TASK-E1-3**: 수집된 궤적과 자택 출발지 좌표를 IndexedDB 로컬 스토리지에만 저장 (6h / Member D)
...
* **세부 Task**:
  ...
  * **TASK-E2-3**: 최근 보행 피드백 누적 통계 IndexedDB 적재 모듈 (4h / Member D)
...
* **세부 Task**:
  ...
  * **TASK-E3-2**: 부정 평가(예: "경사 힘들었음") 수신 시 허용 경사도를 자동 하향하는 무상태 보정 체인 (6h / Member A)
...
#### US-G2: 간편 이메일 가입 및 코스 공유 (출발지 200m 마스킹) (5 pt / 총 24h)
* **담당**: Member E, D | **스프린트**: Sprint 2 (Week 4)
* **세부 Task**:
  * **TASK-G2-1**: Supabase Auth 기반 간편 이메일/비밀번호 가입/로그인 모듈 (Auto-confirm) (6h / Member E)
  * **TASK-G2-2**: 커뮤니티 코스 공유 시 **출발지/도착지 200m 좌표 자동 마스킹(절단/블러링)** 파이프라인 (6h / Member D, E)
  * **TASK-G2-3**: `community_courses` 테이블 적재 및 공개 피드 조회 REST API 구현 (6h / Member E)
  * **TASK-G2-4**: 커뮤니티 피드 뷰 및 코스 좋아요(북마크) UI 컴포넌트 개발 (6h / Member D)
...
* **세부 Task**:
  ...
  * **TASK-H1-2**: 현장 실산책 수행 및 데이터 수집 (계단 회피 적중률, 경사 만족도, 프라이버시 로컬 저장 만족도) (10h / 전원)
...
|---|:---:|:---:|:---:|:---:|
| **Epic A. 산책 조건 입력 및 개인화** | 11 pt | 10개 | 54h | 14.8% |
| **Epic B. 무계단·완만 경사·그늘 우선 경로 생성** | 26 pt | 16개 | 124h | 33.9% |
| **Epic C. 지도 표시 및 외부 내비게이션** | 7 pt | 7개 | 34h | 9.3% |
| **Epic D. 현장 위험 분석 및 재탐색** | 10 pt | 8개 | 48h | 13.1% |
| **Epic E. 산책 기록 및 Memory (Local-First)** | 11 pt | 10개 | 48h | 13.1% |
| **Epic F. 기상/열 위험 정보** | 3 pt | 3개 | 14h | 3.8% |
| **Epic G. 주차 및 커뮤니티 (이메일 Auth/마스킹)** | 7 pt | 6개 | 34h | 9.3% |
| **Epic H. 실사용 검증 및 운영** | 5 pt | 4개 | 30h | 8.2% |
| **합계** | **75 pt** | **64개 Task** | **366h** | **100.0%** |
...
| **Member D** | Frontend / Mobile UX Lead | 86h | 약 17.2h / 주 |
| **Member E** | Infra / Data / QA Lead | 46h | 약 9.2h / 주 |
| **합계** | **5인 전원** | **366h** | **약 73.2h / 주 (팀 전체)** |
