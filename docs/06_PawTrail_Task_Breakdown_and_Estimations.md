...
* **상세 작업 분할 (Task Breakdown)**:
  * **TASK-15-1**: PWA Service Worker 및 Cache API 기반 추천 경로 지도 타일 & GeoJSON 사전 캐싱 모듈 구현 (5h / Member D)
    - 코스 생성 시 해당 경로 주변의 Mapbox 벡터 타일과 GeoJSON 데이터를 브라우저 로컬 캐시에 즉각 프리캐싱.
  * **TASK-15-2**: 네트워크 단절 감지 토스트 UI 및 IndexedDB 로컬 체크인 임시 저장/자동 동기화 구현 (5h / Member D)
    - 오프라인(`navigator.onLine === false`) 상태 안내 토스트 및 오프라인 산책 완료 시 IndexedDB 임시 보관, 네트워크 복구 시 백엔드 DB 자동 동기화.
...
| **US-15** | 오프라인 지도 캐시 및 산책 유지 (PWA) | Member D | 2 pt | 2개 | 10 h |
