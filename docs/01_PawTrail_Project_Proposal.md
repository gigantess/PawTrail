...
## 8. 실사용자(최소 5인) CBT 테스트 및 피드백 반영 계획

* **테스트 대상**: 실제 반려견 견주 5명 이상 (소형견 2, 대형견 1, 노령/관절 안심 케어견 2)
* **테스트 시나리오**:
  1. EAS Build로 배포된 APK 설치 후 반려견 프로필(견종, 체급, 관절 안심 케어 선호) 로컬 등록
  2. **사용자 산책 조건 설정 (예: "계단 없는 완만한 길" 20분 코스 생성 요청)**
  3. 스마트폰을 주머니에 넣고 **핸즈프리 음성 안내를 들으며 안전하게 산책 수행**
  4. 추천 경로에서 계단 회피 및 완만 경사가 적절히 반영되었는지 산책 수행
  5. 산책 중 현장 위험물(높은 턱/공사) 사진 1회 촬영·업로드하여 비전 판독 검증
  6. 산책 종료 후 나만의 코스 즐겨찾기 저장 및 피드백 설문(음성 안내 편의성, 코스 만족도) 제출
* **피드백 반영 체계**:
  - 수집된 피드백을 바탕으로 경사도 가중치($F_{\text{slope}}$), 그늘 지표 가중치($F_{\text{shade}}$) 및 에이전트 프롬프트 즉각 수정
